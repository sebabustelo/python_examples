...
class FabricaFirulais:
    def __init__(self):
        self.materia_prima = 100  # Costo de la materia prima (valor ficticio para ejemplo)

    def calcular_costo_mano_obra(self, codigo):
        if codigo in [3, 4]:
            return self.materia_prima * 0.75
        elif codigo in [1, 5]:
            return self.materia_prima * 0.85
        elif codigo in [2, 6]:
            return self.materia_prima * 0.90
        else:
            return 0

    def calcular_gasto_fabricacion(self, codigo):
        if codigo in [2, 5]:
            return self.materia_prima * 0.30
        elif codigo in [3, 6]:
            return self.materia_prima * 0.35
        elif codigo in [1, 4]:
            return self.materia_prima * 0.28
        else:
            return 0

    def calcular_precio_venta(self, codigo):
        costo_mano_obra = self.calcular_costo_mano_obra(codigo)
        gasto_fabricacion = self.calcular_gasto_fabricacion(codigo)
        costo_produccion = self.materia_prima + costo_mano_obra + gasto_fabricacion
        precio_venta = costo_produccion + (0.45 * costo_produccion)
        return precio_venta

def main():
    fabrica = FabricaFirulais()

    codigo_articulo = int(input("Ingrese el código del artículo (1-6): "))
    if codigo_articulo < 1 or codigo_articulo > 6:
        print("Código inválido.")
    else:
        precio_venta = fabrica.calcular_precio_venta(codigo_articulo)
